...
CalAnimationAction::CalAnimationAction(CalCoreAnimation* pCoreAnimation): CalAnimation(pCoreAnimation)
{
  setType(TYPE_ACTION);
  mfTotalTime = pCoreAnimation->getDuration();  // This is a modification to fit into engin3d!
}
 /*****************************************************************************/
/** This Method it´s a expansión of the original code to fit into engin3d!.
  *
  * This function removes something of the animation action instance.
  *****************************************************************************/
...
bool CalAnimationAction::update(float deltaTime)
{
  ...
  if(getState() == STATE_OUT)
  {
    // check if we are still in the OUT phase 
	  // THIS IS A MODIFICATION TO FIT INTO Engin3D! replaced: "getCoreAnimation()->getDuration()" with: "mfTotalTime"
